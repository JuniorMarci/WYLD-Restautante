...
// Carregar os dados
$inventario = carregarDados('inventario.json');
$menu = carregarDados('menu.json');
$ventas = carregarDados('ventas.json');
$compras = carregarDados('compras.json');
$proveedores = carregarDados('proveedores.json');
?>
...
// Adicionar esta função no início para facilitar o uso
function guardarDatosPHP(archivo, datos) {
    console.log(archivo);
    console.log(datos);
    
    // Converter datos para JSON string se não for string
    const datosEnviar = typeof datos === 'string' ? datos : JSON.stringify(datos);
    
    // Enviar os dados para o PHP via POST
...
let compras = <?php echo json_encode($compras ?: []); ?>;
...
function registrarCompra() {
	...
	actualizarTablaCompras()
	guardarDatosPHP('compras.json', compras);
...
function guardarBar() {
	...
	guardarDatosPHP('menu.json', menu);
    actualizarTablaBar();
...
function agregarProveedor() {
	...
	guardarDatosPHP('proveedores.json', proveedores);
...
function buscarProveedores() {
	...
	guardarDatosPHP('inventario.json', inventario);
...
function agregarVenta() {
    ...
    limpiarVenta();
	guardarDatosPHP('ventas.json', ventas);
	guardarDatosPHP('inventario.json', inventario);
}
